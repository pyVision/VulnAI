...
class TransformerModelHandler:
    ...
    def run(self, input_text):
        ...
        decoded_output = self.decode(outputs[0], skip_special_tokens=True)
    ...
    def decode(self, outputs):
        """
        Decode the output tensor from the model into a human-readable string.

        Args:
            outputs (torch.Tensor): The output tensor generated by the model.

        Returns:
            str: The decoded output text, with special tokens skipped.
        """
        # Decode the output tensor to a readable string, skipping special tokens
        decoded_output = self.tokenizer.decode(outputs, skip_special_tokens=True)
        return decoded_output  # Return the decoded output
